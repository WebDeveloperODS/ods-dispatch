...
export default function Header() {
  ...
  const [portfolioCheck , setPortfolioCheck] = useState(false)
  ...
  useEffect(() => {
    if(location.pathname === '/carriers-setup' || location.pathname === '/application-to-avail-services'){
      setCarriersCheck(true)
      setPortfolioCheck(false)
    } else if(location.pathname === '/portfolio'){
      setPortfolioCheck(true)
      setCarriersCheck(false)
    }else {
      setCarriersCheck(false)
      setPortfolioCheck(false)  
    }
  ...
  return (
    ...
    <div className="relative flex flex-col">
      ...
      <div className={`w-full transition-all ease-in-out duration-300 ${callSticky ? 'fixed top-0 z-50 bg-white py-1 translate-y-0 shadow-sm shadow-neutral-300': carriersCheck ? 'top-0 relative':'absolute top-0 lg:top-7 z-10'} left-0 right-0 `}>
        <div className={`container flex justify-between items-center`}>
          ...
          <nav className="hidden lg:block">
            <ul className={`flex items-center justify-center gap-5 ${portfolioCheck && callSticky === false? 'backdrop-blur-lg p-3 rounded-xl border border-neutral-400 shadow-md': null}`}>
              {HeaderMenu.map((menu, index) => (
                <li key={index} 
                  ...
                  <a
                    className={`${callSticky || carriersCheck || portfolioCheck ? 'text-neutral-800' :'text-neutral-100 text-shadow-lg text-shadow-neutral-700'} flex items-center justify-center gap-2 font-bold text-sm uppercase ${ location.pathname === menu.link ? 'border-b-2 border-red-700 font-bold! text-shadow-md':' '} hover:border-b-2 hover:border-red-800 transition-all duration-200`}
                    href={menu.link}
                  >
                    {menu.title} 
                    {menu.childPages && (
