export function H1({children,extraCss}){
    return(
        <h1 className={`${extraCss} text-2xl xl:text-3xl font-bold uppercase tracking-normal`}>{children}</h1>
    )
}
export function H2({children,extraCss}){
    return(
        <h2 className={`${extraCss} text-xl xl:text-2xl font-bold tracking-normal`}>{children}</h2>
    )
}
