...
import { ChevronRight, ChevronRightCircle, MailIcon, PhoneCall, X } from 'lucide-react'
...
const SideBar = ({removeWhite}) => {
    ...
    return (
    <>
        <div onClick={() => setOpen(true)} className={`border-2 p-1 ${removeWhite ? 'border-red-700 hover:border-transparent': 'border-neutral-100'} rounded-xl cursor-pointer hover:bg-red-700 transition-all ease-in-out duration-300 hover:scale-[1.05] hover:border-transparent`}>
            <CgMenuRight className={`h-6 w-auto ${removeWhite ? 'text-red-800 hover:text-neutral-100' : 'text-neutral-100'} stroke-1`}/>
        </div>
        <div className={`fixed top-0 left-0 bg-neutral-800/60 w-screen h-screen z-1000 transition-all ease-in-out duration-500 ${open ? 'translate-x-0':'-translate-x-full'}`}>
            <div className={`fixed w-84 h-screen bg-neutral-100 flex flex-col transition-all ease-in-out duration-800 ${open ? 'translate-x-0':'-translate-x-full'}`}>
                ...
                <img className='h-44 w-auto mx-auto my-5' src='/images/logo-3.png'/>
                <ul className='flex flex-col px-10 gap-3'>
                    {
                        HeaderMenu.map((item,index) => 
                            <li key={index}>
                                ...
                                key={index} onMouseEnter={() => setHover(index)} onMouseLeave={() => setHover(-1)} 
                ...
                <div className='fixed flex flex-col px-10 gap-3 bottom-10 left-0 right-0'>
                    <a className="flex items-center tracking-wide font-semibold justify-between rounded-xl px-3 py-2 cursor-pointer text-white bg-blue-900 hover:bg-slate-900"
                        href={''}>
                        (971) 472 7066
                        <MdOutlinePhoneIphone className="h-5 w-auto mt-[0.175em]" /> 
                    </a>
                    <a className="flex items-center tracking-wide font-semibold justify-between rounded-xl px-3 py-2 cursor-pointer text-white bg-red-700 hover:bg-red-950"
                        ...
                        <MailIcon className="h-5 w-auto mt-[0.175em]" /> 
                    </a>
                </div>
            </div>
        </div>
